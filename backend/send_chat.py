...
def lambda_handler(event, context):
    ...
    if is_stream:
        ...
        if result:
            ...
            cur.execute("INSERT INTO chats (user_id, post_by, post_at, text, imageurl) VALUES (%s, %s, %s, %s, %s)", (user_id, "assistant", timestamp, message, imageurl))
    else:
        if imageurl:
            cur.execute("INSERT INTO chats (user_id, post_by, post_at, text, imageurl) VALUES (%s, %s, %s, %s, %s)", (user_id, "assistant", timestamp, message, imageurl))
        else:
            cur.execute("INSERT INTO chats (user_id, post_by, post_at, text) VALUES (%s, %s, %s, %s)", (user_id, "assistant", timestamp, message))
    conn.commit()
    print("commit finished.")

    # ユーザにメッセージを送信
    cur.execute("SELECT session_id FROM users WHERE user_id = %s", (user_id,))
    user_session_id = cur.fetchone()
    print("got cur.fetchone", type(user_session_id), f"{user_session_id=}")
    try:
        apigw.post_to_connection(ConnectionId=user_session_id[0], Data=json.dumps({'message': message, 'imageurl': imageurl, 'is_stream': is_stream}))
    except Exception as e:
        logger.error(f"Error sending message to connection {user_session_id}: {e}")
